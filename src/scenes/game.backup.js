...
class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.testBody = this.matter.add.circle(150, 250, 16);
    ...
    // this.matter.add.collider(this.player, this.allPlayersGroup);
  ...
  update() {
    ...
    // if (this.player) {
    //   var closest = this.matter.closest(this.player);
  ...
  updatePlayers(data) {
    ...
    Object.keys(data).forEach((id) => {
      if (this.allPlayers[id] && id != this.player.id) {
        ...
        // // this.matter.add.collider(newPlayer, this.platforms);
        // // this.matter.add.collider(this.player, newPlayer);
